...
class StahlLikeCalibrator:
    ...
    def __init__(self, camera, feature_finder, **kwargs):
        ...
        self.default_pixels_per_mm = kwargs.get('default_pixels_per_mm', 17.96)                     # Not sure about this setting
        self.default_Rp = kwargs.get("default_Rp", 3.2 * self.default_pixels_per_mm)                # May need to change for shrew dimensions

        self.ui_queue = kwargs.get("ui_queue", None)
        self.x_image_axis = kwargs.get("x_image_axis", 1)       # Note for coordinates, [row, col] = [y, x] convention is followed
        self.y_image_axis = kwargs.get("y_image_axis", 0)       # Note for coordinates, [row, col] = [y, x] convention is followed
        ...
        self.jog_angle = kwargs.get("jog_angle", 15.)       # angle of camera rotation for pseudo-eye rotation in degrees  
        ...
        self.Dx = kwargs.get("Dx", 1)                       # a quantum of stage displacement in millimeters

        self.default_cr_positions = {}
        self.cr_tolerances = 5         # In pixels. Value needs to be determined. Used as a metric of how well the cr is aligned with axes.
        self.pos_guess = {"pupil_position": array([0, 0]), "cr_position": array([0, 0])}
