...
class CameraReader:
    ...
    def __init__(self, cameraID, animalName, **kwargs):
        ...
        self.save_vid = kwargs.get("save_vid", False)               # Save video or no
        ...
        if self.save_vid:
            self.frameRate = 30                                             # Set framerate at which video is saved
            fourcc = cv2.cv.CV_FOURCC(*'XVID')                              # Save as .avi with XVID encoding
...
if __name__ == '__main__':
    ...
    cameraID = 0
    savePath = 'C:/users/mccannm/Desktop/video' + str(cameraID) + '.avi'
    im_q = Queue()   

    get_roi = ROISelect(cameraID)
    get_roi.findROI()
    verticesROI, frame_size, pupil, cr = get_roi.getData()
    cr = CameraReader(cameraID, "Dummy", add_axes=False, image_queue=im_q, vidPath=savePath, roi=verticesROI, only_roi=True)
