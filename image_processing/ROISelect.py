...
# Simply click and drag to select ROI. Press R to reset selection, and enter to crop, and ESC to exit.
# Once ROI is selected, press ENTER to select pupil and corneal LED reflection (if present).
# After pupil and CR identified, the user can click to fit an ellipse to the edge of the eyeball to create a mask used during image adjustment.
...
# Click to make polygon adapted from http://stackoverflow.com/questions/37099262/drawing-filled-polygon-using-mouse-events-in-open-cv-using-python
...
class ROISelect:
    def __init__(self, cameraID, **kwargs):
        # setup super basic camera reader to grab cam frames
    ...
    def findROI(self):
        """Click and drag to select ROI. Press R to reset selection, ESC to abort, and Enter to crop. """
        self._get_image()
        ...
        if len(self.refPt) == 2:
            ...
            x = self.refPt[1][0]
            y = self.refPt[1][1]
            self.refPt = [(min(ix, x), min(iy, y)), (max(ix, x), max(iy, y))]  # set bounding box by mouse move
            self.roi_diff = [(min(iy, y)), (min(ix, x))]                       # stick to [rows, columns] convention (Y, X)
    ...
    def findEyeEllipse(self):
        ...
        while not self.poly_select:
            ...
            if key == 13 and len(self.elli_pts) >= 5 and not is_fit and not is_confirmed and not self.poly_select:
                is_fit = True
            ...
            elif key == 13 and len(self.elli_pts) < 5 and not is_fit and not is_confirmed and not self.poly_select:
                pass
            ...
            # Line reset
            elif key == ord('r') and not self.poly_select and not is_fit and not is_confirmed:
            ...
            # ellipse reset
            elif key == ord('r') and not self.poly_select and is_fit and not is_confirmed:
                is_fit = False
                self.roi = self.empty_roi.copy()
    ...
    def create_eyeball_mask(self):
        ...
        _, im1 = cv2.threshold(self.roi, int(im_val_pup * 0.9), 255, type=cv2.THRESH_TOZERO_INV)
    ...
    def getData(self):
        # calculate distance from edges of main frame to ROI for future operations (i.e. CR and pupil center)
        # Everything output to form [(x,y)]
