...
class SubpixelStarburstEyeFeatureFinder:
    ...
    def __init__(self, **kwargs):
        ...
        # imaging modality set from dropdown in shrewdriver
        self.modality = kwargs.get("modality", '2P')  # CHOOSE IMAGING MODALITY
        ...
        if self.modality == '2P':
            ...
            self.pupil_threshold = kwargs.get('pupil_threshold', 20)
            self.cr_threshold = kwargs.get('cr_threshold', 100)
    ...
    def analyze_image(self, image_list, guess, **kwargs):
        ...
        # If no guess is provided, pick the image center as the guess <----- MM 11/1/16
        ...
        if self.shortcut_sobel is None:
            ...
            for im in ds_image:
                ...
                image_grad_mag += (image_grad_mag1 // len(ds_image))    # <------------- take average of sobel transforms  MM 12/15/2016
        ...
        # Pack up the results
        # Note that all position results follow the [rows, columns] convention. This means the formatting in Cartesian coordinates is [Y, X]
        try:
    ...
    def _find_ray_boundaries(self, im, seed_point, zero_referenced_rays, cutoff_index, threshold, **kwargs):
        ...
        if 'exclusion_center' in kwargs:
            ...
            if exclusion_center.any() != -1:
                im[int(exclusion_center[self.y_axis]) - int(exclusion_radius) : int(exclusion_center[self.y_axis]) + int(exclusion_radius),
                    int(exclusion_center[self.x_axis]) - int(exclusion_radius) : int(exclusion_center[self.x_axis]) + int(exclusion_radius)] = 0
        ...
        # scan inward-to-outward to find the first threshold crossing if looking for cr
        # need to find second threshold if looking at shrew pupil during 2P
        if self.modality == '2P':
            for r in range(0, vals.shape[0]):
                ...
                for v in range(cutoff_index, vals.shape[1]):
                    ...
                    if ray_type == 'pupil':
                        ...
                        # We want a more robust cutoff than just a positive or negative derivative. Set one much greater than zero
                    ...
                    if ray_type == 'cr':
                        ...
                        if val > threshold - 0.2*threshold and val < threshold + 0.2*threshold:
                            crossed = True

                        # We want a more robust cutoff than just a positive or negative derivative. Set one much less than zero
                        if crossed and vals_slope[r, v] <= d_cr:
                            boundary_points.append(array([rays_x[r, v - 1], rays_y[r, v - 1]]))
                            break

        else:
            # Original code from Cox et al.
            for r in range(0, vals.shape[0]):
                crossed = False
                ...
                for v in range(cutoff_index, vals.shape[1]):
                    ...
                    if crossed and vals_slope[r, v] <= -5:
                        boundary_points.append(array([rays_x[r, v - 1], rays_y[r, v - 1]]))
                        break
    ...
    def _get_image_values_interp(self, im, x, y):
        ...
        for i in range(0, x.shape[0]):
            for j in range(0, x.shape[1]):
                if floor_x[i, j] < 0 or floor_y[i, j] < 0 or ceil_x[i, j] > im.shape[0] or ceil_y[i, j] > im.shape[1]:
                    vals[i, j] = nan
                    continue
    ...
    def _get_image_values_interp_faster(self, im, x, y):
        ...
        vals = x_frac * y_frac * a + (1 - x_frac) * y_frac * b + x_frac * (1 - y_frac) * c + (1 - x_frac) * (1 - y_frac) * d
    ...
    def _fit_circle_to_points_lstsq(self, points):
        ...
        a0 = -2 * center_guess[0]
        b0 = -2 * center_guess[1]
        c0 = center_guess[0] ** 2 + center_guess[1] ** 2 - (points_array[:, 0] - center_guess[0]).mean() ** 2
        p0 = array([a0, b0, c0])

        output = scipy.optimize.leastsq(self._residuals_circle, p0, args=(points_x, points_y))
    ...
    def _fit_circle_to_points_lstsq_ransac(self, points):
        ...
        while iter < max_iter:
            ...
            if len(maybe_inliers) > good_fit_consensus:
                candidate_center, candidate_radius, candidate_err, _, _ = self._fit_circle_to_points_lstsq(array(maybe_inliers))
