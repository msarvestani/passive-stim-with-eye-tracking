...
# ---------- Stimulus Parameters ---------- ##
timepertrial = 1 # Duration for tiral, stimulus and ISI # 26s for single trial
stitime = 0.5
ISItime = 0.5
Trinumber = 80 #repeat #Run all the stims this many times
Imasize = [814, 814] # size of image by pix
centerPoint = [113,150] 
changeDirectionTimeAt = .25
degPerFrame = .01
direc = 90

# Set the random sequence
items = range(1,181) #For 512 images
...
flipStim = visual.PatchStim(win=win,tex=barTexture,mask='none',units='pix',pos=centerPoint,size=(814,814))
...
image = visual.ImageStim(win=win, name='image',units=u'pix', 
    image='sin', mask=None,
    ori=0, pos=centerPoint, size=Imasize,
...
trials = data.TrialHandler(nReps=Trinumber, method=u'random', 
    ...
    trialList=data.importConditions(u'condition4.xlsx'),
...
for thisTrial in trials:
    ...
    #Iname='C:/Users/fitzlab1/Documents/psychopy/Joe/exp/tiffDir_GaussWN/wn'+str(1)+'_' +'{:04.0f}'.format(aa)+ '.tif'
    Iname='C:/Users/fitzlab1/Documents/psychopy/Joe/exp/bartlett_edge4_comb/a'+'{:04.0f}'.format(aa)+ '.tif'

    #Iname = 'C:/Users/fitzlab1/Documents/psychopy/Gordon/episodicStims/WaveletStims/tiffDir_GaussWN/wn'+str(1)+'_' + '{:04.0f}'.format(aa)+ '.tif'
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *image* updates
        if t < stitime and image.status == NOT_STARTED:
            # keep track of start time/frame for later
            image.tStart = t  # underestimates by a little under one frame
            image.frameNStart = frameN  # exact frame index
            #image.setAutoDraw(True)
            if t>changeDirectionTimeAt:
                image.setPos([degPerFrame*math.degrees(math.cos(math.radians(direc-180))),degPerFrame*math.degrees(math.sin(math.radians(direc-180)))],'+', 'deg')
            else:
                # present stimulus for this trial
                image.setPos([degPerFrame*math.degrees(math.cos(math.radians(direc))),degPerFrame*math.degrees(math.sin(math.radians(direc)))],'+', 'deg')
            image.draw()
