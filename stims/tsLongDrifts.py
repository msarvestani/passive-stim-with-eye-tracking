...
if sys.argv[-1] == '--called':
    isCalled = True
    dataPath = sys.argv[-2]
    logFilePath = dataPath + "_abs_stim_on_times.txt"
...
# trials and duration
# orientations = [0, 45, 90, 135, 180, -135, -90, -45]
...
changeDirectionAt = 1  # When do we change movement directions? If 1, there should be no reversal.  If 0.5, then movement reverses directions at
# stimDuration/2
isi = 2  # Kuo 4 to 2
isRandom = 1
# Grating parameter
...
stimCodeName = os.path.dirname(os.path.realpath(__file__)) + '\\' + os.path.basename(__file__)
