...
# preview_fit.py: Preview Starburst Fit User Inerface
# Author: Matthew McCann (@mkm4884)
# Max Planck Florida Institute for Neuroscience
# Created: 12/14/2016
# Last Modified:

# Description: A GUI to preview pupil tracking fit before beginning a training session. Interacts closely
# with the ShrewDriver script. The user can accuracy of the feature finder by adjusting the gamma, contrast,
# and brightness of the raw images, and the search parameters of the starburst feature finder. A live preview
# is displayed. When results are satisfactory, the output is saved to a text file to be loaded by ShrewDriver.
...
class PreviewFitUI(QtGui.QMainWindow, Preview_class):
    ...
    def __init__(self, parent=None):
        ...
        if sys.argv[-1] == '--called':
            self.wasCalled = True
            # Unpickle guess dictionary for initial guess, cameraID, and ROI if it exists
            # This file will only be present if this file was called as a subprocess
            pkl_file = open('data.pkl', 'rb')
            self.guess = pkl.load(pkl_file)
            self.base_path = self.guess['basepath']     # This will be a path to a video file for offline analysis and to the main data directory for online
            ...
            # These variables are present when calling from shrewdriver. From here we can define the current session number and write videos and text data
            # here. If this variable does not exist, is assigned a None value
            ...
            del self.guess['basepath'], self.guess['animalID'], self.guess['cameraID'], self.guess['ROI']
        ...
        ##-- IF WE ARE CALLING THE FUNCTION AS A STANDALONE ENTITY --##
            ...
            self.base_path = os.getcwd()   # Change this based on local machine if running in standalone format
            self.animalName = 'Test'
            self.cameraID = '0'       # Change this based on hardware setup of computer
            get_roi = ROISelect.ROISelect(self.cameraID)
            get_roi.findROI()
            self.ROI, self.roi_diff, pupil, cref, ellipse = get_roi.getData()
            self.guess = {'pupil_position': array(pupil[0]), 'cr_position': array(cref[0])}
            self.guess_i = self.guess.copy()
            self.save_vid = False
            self.save_log = False

        # Depending on if we are doing offline or online analysis, the data folder changes
        if self.cameraID != 'recorded':
            self.online = True
            self.base_path = self.base_path + '/' + self.animalName + '/'      # change this for local machine
            self.temp_data_path = self.base_path + '/' + 'temp_image_settings'
            ...
            self.calibratorPath = self.base_path.rsplit('/', 2)[0]
        ...
        self.starburst_ff = SubpixelStarburstEyeFeatureFinder.SubpixelStarburstEyeFeatureFinder(modality='2P', pupil_n_rays=self.pupil_n_rays,
                                                              pupil_ray_length=self.pupil_ray_length, pupil_threshold=self.pupil_threshold,
                                                              cr_n_rays=self.cr_n_rays, cr_ray_length=self.cr_ray_length, cr_threshold=self.cr_threshold)

        self.ff_pipe = PupilLocator(self.frame_queue, self.starburst_ff, self.guess, self.vidPath, ROI=self.ROI, ROI_diff=self.ROI_diff,
                                    save_vid=False, save_log=True, online=self.online, ellipse=ellipse)
        ...
        self.slGamma.valueChanged.connect(lambda: self.change_slider_spin(self.slGamma, self.spnGamma, self.gamma, 'gamma'))
        self.slContrast.valueChanged.connect(lambda: self.change_slider_spin(self.slContrast, self.spnContrast, self.contrast, 'contrast'))
        self.slBrightness.valueChanged.connect(lambda: self.change_slider_spin(self.slBrightness, self.spnBrightness, self.brightness, 'brightness'))

        # spin box actions (with slider change)
        self.spnGamma.valueChanged.connect(lambda: self.change_spin_slider(self.spnGamma, self.slGamma, self.gamma, 'gamma'))
        self.spnContrast.valueChanged.connect(lambda: self.change_spin_slider(self.spnContrast, self.slContrast, self.contrast, 'contrast'))
        self.spnBrightness.valueChanged.connect(lambda: self.change_spin_slider(self.spnBrightness, self.slBrightness, self.brightness, 'brightness'))

        # spin box actions (no slider change)
        self.spnSobelFrame.valueChanged.connect(lambda: self.change_spin(self.spnSobelFrame, self.num_frames_sobel, 'num_frames'))
        self.spnPupilRays.valueChanged.connect(lambda: self.change_spin(self.spnPupilRays, self.pupil_n_rays, 'pupil_n_rays'))
        self.spnPupilRayLength.valueChanged.connect(lambda: self.change_spin(self.spnPupilRayLength, self.pupil_ray_length, 'pupil_ray_length'))
        self.spnPupilThresh.valueChanged.connect(lambda: self.change_spin(self.spnPupilThresh, self.pupil_threshold, 'pupil_threshold'))
        self.spnCRRays.valueChanged.connect(lambda: self.change_spin(self.spnCRRays, self.cr_n_rays, 'cr_n_rays'))
        self.spnCRRayLength.valueChanged.connect(lambda: self.change_spin(self.spnCRRayLength, self.cr_ray_length, 'cr_ray_length'))
        self.spnCRThresh.valueChanged.connect(lambda: self.change_spin(self.spnCRThresh, self.cr_threshold, 'cr_threshold'))
    ...
    def load_calibration_file(self):
        self.calibratorPath = self.calibratorPath + "/calibrator/"
    ...
    def start_camera(self):
        ...
        if self.cameraID != 'Point_Grey' and self.cameraID != 'recorded':
            try:
                self.CameraReader = CameraReader(int(self.cameraID), self.cameraID, vidPath=self.vidPath, roi=self.ROI, display_vid=True,
        ...
        # If no observation camera, keep frame rate high
            ...
            try:
                mode = 1
                self.CameraReader = runGrasshopper(mode, vidPath=self.vidPath, roi=self.ROI, animalName=self.animalName, display_vid=True,
            ...
            try:
                self.CameraReader = VideoReader(self.vidPath, image_queue=self.frame_queue, enable_display=False, roi=self.ROI)
                self.CameraReader.startReadThread()
            except:
    ...
    def stop_camera(self):
        if self.CameraReader is not None and self.cameraID != 'Point_Grey' and self.cameraID != 'recorded':
